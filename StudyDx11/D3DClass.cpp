...
// vsync表示是否需要同步到显示器的刷新频率，不需要的话就会尽可能快的刷新
// hwnd为窗口句柄用于dx获取创建的窗口的信息
	...
	float screenDepth, float screenNear)
{
	...
	// 用于枚举显卡第一个参数代表了当前正在枚举第几个显卡，获取到的显卡设备放入adapter
	...
	for (i = 0; i < numModes; i++)
	{
		if (displayModeList[i].Width == (unsigned int)screenWidth)
		{
			...
			{
				numerator = displayModeList[i].RefreshRate.Numerator; // 分子可能是是60
				denominator = displayModeList[i].RefreshRate.Denominator; // 分母可能是1
			}
		}
	}
	...
	// 设置交换链的频率，根据之前设置的是否需要垂直同步
	// Set the refresh rate of the back buffer.
	if (m_vsync_enabled)
	{
		// 当使用垂直同步时需要将刷新率设置成和之前获取的一样
	...
	// 设置backbuffer的用法
	// 还有一种是DXGI_USAGE_BACK_BUFFER表示离屏渲染比如保存一张图片到文件
	// 猜测这时只有back没有front，而DXGI_USAGE_RENDER_TARGET_OUTPUT是标准的交换链用法
	...
	// 设置功能级别还可以设置成dx9或dx10的用以支持一些老设备。
	featureLevel = D3D_FEATURE_LEVEL_11_0;

	// Create the swap chain, Direct3D device, and Direct3D device context.
	// D3D_DRIVER_TYPE_HARDWARE可以用D3D_DRIVER_TYPE_REFERENCE替代如果你没有支持dx11的显卡的话
	// 这里创建了交换链，D3D设备，D3D上下文设备
	...
	// 将渲染目标视图和深度以及模板缓冲绑定到了渲染流水线的输出
	// 这样渲染流水线将图像绘制到backbuffer，然后再通过交换链将其呈现到屏幕
	// Bind the render target view and depth stencil buffer to the output render pipeline.
	m_deviceContext->OMSetRenderTargets(1, &m_renderTargetView, m_depthStencilView);

	// 到现在为止渲染目标就设置好了，还有些额外的函数可以使得我们控制场景。
	// 比如光栅器状态，将控制多边形怎样被渲染。我们可以设置成线框渲染模式，或者双面多边形。
	// 默认的光栅器状态是如下的设置
...
// 获取d3d设备和d3d上下文设备，这两个设备就是以前dx9里的一个d3d设备
